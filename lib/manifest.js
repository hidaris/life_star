...
function buildFindCommand(patterns) {
  ...
  var findPatternExpr = '\\( -iname "' + patterns.join('" -o -iname "') + '" \\)',
      cmd = 'find .'
          + ' \\( -name node_modules -or -name .git \\) -type d -prune'
          + ' -o ' + findPatternExpr;
  return cmd;
}
...
console.log(buildFindCommand(manifestFilePattern) + ' -print');
  ...
  lastModificationDateOfFilesIn(dir, function(err, lastMod) {
    ...
    findJSFilesForManifest(dir, function(err, filesString) {
      ...
      manifestContent = "CACHE MANIFEST\n# timestamp " + lastMod + "\n\n"
                      + 'CACHE:\n' + filesString
                      // + '/favicon.ico\n'
                      // + '/core/media/loading.gif\n'
                      // + '/core/media/morphloading.gif\n'
                      + '\n\nNETWORK:\n'
                      + '*\nhttp://*\nhttps://*\n';
...
function addManifestRefToHTMLSource(sourceOrBuffer) {
  var s = sourceOrBuffer.toString();
...
ManifestHandler.prototype.addManifestRef = function(req, res) {
  ...
  function addManifestAttributeToResponseBody(buffers) {
	...
	if (idx === -1) continue;
	var manifestRef = ' manifest="/' + manifestFileName + '"';
