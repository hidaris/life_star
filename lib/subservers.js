...
SubserverHandler.prototype.withSubserver = function(req, res, func) {
  var name = req.params.name, subserver = this.getSubserver(name);
  try {
    if (!subserver) { res.status(404).end({error: 'subserver not found'}); return; }
    func.call(this, subserver);
  } catch(e) {
    console.error(e);
...
SubserverHandler.prototype.registerWith = function(app) {
  ...
  app.get(metaURL + '/:name/source', this.getSubserverSource.bind(this));
  app.put(metaURL + '/:name/source', this.setSubserverSource.bind(this));
