  ...
  req.on('end', function(err) {
    try {
      var jso = JSON.parse(data),
          subserver = self.getSubserver(jso.name);
      if (!subserver) { res.status(404).end({error: 'subserver not found'}); return }
      if (!self.app) { res.status(500).end({error: 'SubserverHandler has no app'}); return }
      subserver.unload(self.app);
      res.end();
    } catch(e) {
      console.error(e);
      res.status(500).end(String(e));
...
SubserverHandler.prototype.registerWith = function(app) {
  ...
  app.post(metaURL + '/unload', this.unload.bind(this));
